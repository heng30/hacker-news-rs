...
impl HnClient {
    ...
    pub async fn search_newest(&self, keyword: &str, limit: usize) -> Result<Vec<HnStory>> {
        ...
        tracing::debug!("Algolia response text (first 500 chars): {}", text.chars().take(500).collect::<String>());

        let algolia: AlgoliaResponse = serde_json::from_str(&text)
            .map_err(|e| {
                tracing::error!("Failed to parse Algolia response: {}. Response text: {}", e, text.chars().take(1000).collect::<String>());
                anyhow::anyhow!("Failed to parse Algolia response: {}", e)
            })?;

        tracing::info!("Algolia returned {} hits for keyword '{}'", algolia.hits.len(), keyword);

        Ok(algolia.hits.into_iter().map(|h| {
            HnStory {
