...
pub async fn get_story(hn_id: i64) -> Result<Option<Story>, ServerFnError> {
    let state = app_state()?;
    crate::db::get_story_by_hn_id(&state.db, hn_id).map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn delete_all_stories() -> Result<usize, ServerFnError> {
    let state = app_state()?;
    crate::db::delete_all_stories(&state.db).map_err(|e| ServerFnError::new(e.to_string()))
}
